...
def get_month_hour_data_for_building(data, building_to_search, column, percentage_value=False):
    ...
    for month in months:
        ...
        for hour in hours:
            values = {}
            aps_data_hour = aps_data[aps_data['dataPomiaru'].str.contains(r"\d{4}-" + current_month + r"-\d{2}" + "--" + hour)]
            ...
            """for ap in aps_to_search:
                ap_data = aps_data[aps_data['apName'] == ap]
                ap_data_hour=ap_data[ap_data['dataPomiaru'].str.contains(
                            r"\d{4}-" + current_month + r"-\d{2}"+"--"+hour)]
                for index, adh in ap_data_hour.iterrows():
                    if adh['dataPomiaru'] not in values:
                        values[adh['dataPomiaru']] = 0
                    if str(adh[column]) != 'nan':
                        values[adh['dataPomiaru']] += adh[column]
                        ap_count+=1
                        if percentage_value and adh[column] > max_val:
                            max_val= adh[column]
                if ap == aps_to_search[-1]:
                    summary[current_month][hour]["avg"] = (sum([v for v in values.values()]) / ap_count) if ap_count != 0 else 0
                    if percentage_value:
                        summary[current_month][hour]["max"] = max_val
                    else:
                        summary[current_month][hour]["max"] = max([v for v in values.values()]) if len(values.values()) else 0
                #values[ap] = [cv for cv in ap_data_hour[column].tolist() if str(cv) != 'nan']
                summary[current_month][hour][ap] = {}
                summary[current_month][hour][ap]["max"] =  max(column_values) if len(column_values) else 0
                summary[current_month][hour][ap]['min'] = min(column_values) if len(column_values) else 0
                summary[current_month][hour][ap]['avg'] = (sum(column_values) / len(column_values)) if len(
                    column_values) else 0"""
...
def get_or_generate_month_hour_file_for_buiding(data, building_to_search, column_a, column_b=None, percentage_value=False):
    gen_hours()
    filename = column_a
    filename += ('-' + column_b) if not column_b is None else ''
    filename+="-" + building_to_search
    ...
    try:
        ...
        generated_data = (get_month_hour_data_for_building(data, building_to_search, column_a, percentage_value)
                          if column_b is None else get_month_hour_ratio_for_building(data, building_to_search,column_a,column_b))
        with open(filename, 'w') as generated_file:
